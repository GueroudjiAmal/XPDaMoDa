...
do
    ...
    DATE=$(date +"%Y-%m-%d_%T")
    WORKSPACE=/eagle/radix-io/agueroudji/Prediction/D${DATE}_R${R}_W${NWORKERS}/
    mkdir  -p $WORKSPACE
    cd $WORKSPACE
    cp -r  $DIR/*.py $DIR/Apps/ResNet_prediction.py $DIR/scripts/* $DIR/*.json $DIR/*txt $DIR/custom/*.so $DIR/plugins/* .
