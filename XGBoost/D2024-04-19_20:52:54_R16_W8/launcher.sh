...
for R in 16 35 48 47 ;  #$(seq 1 $RUNS);
do
    ...
    DATE=$(date +"%Y-%m-%d_%T")
    WORKSPACE=/eagle/radix-io/agueroudji/XGBoost/D${DATE}_R${R}_W${NWORKERS}/
    mkdir  -p $WORKSPACE
    cd $WORKSPACE
    cp -r  $DIR/*.py $DIR/Apps/XGBoost.py $DIR/scripts/* $DIR/*.json $DIR/*txt $DIR/custom/*.so $DIR/plugins/* .
